...
def _build_samus(
    ...
    if checkpoint is not None:
        ...
        try:
            ...
            new_state_dict = load_from_pretrained(samus, state_dict, image_size, patch_size)
...
def load_from_pretrained(samus_w_prompt, samus_dict, image_size, patch_size): # load the positional embedding
    samus_w_prompt_dict = samus_w_prompt.state_dict()
    new_samus_dict = {}
    for k,v in samus_dict.items():
        if k[:7] == 'module.':
            new_samus_dict[k[7:]] = v
        else:
            new_samus_dict[k] = v
    dict_trained = {k: v for k, v in new_samus_dict.items() if k in samus_w_prompt_dict}
    samus_w_prompt_dict.update(dict_trained)
    return samus_w_prompt_dict
