...
class ImageEncoderViT_sam(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #x = self.input_Adapter(x) # b h w c
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if window_size == 0:
            ...
            #self.trans_Adapter = Adapter(dim)
            self.scale = 0.5
        # ---------------------------------------------------
        self.dim = dim
...
class AugBlock(nn.Module):
    ...
    def __init__(
        ...
        # ------------------ new to sam----------------------
        if window_size == 0:
            self.MLP_Adapter = AugAdapter(dim, skip_connect=False, num_heads=num_heads)  # new to sam, MLP-adapter, no skip connection
            self.Space_Adapter = AugAdapter(dim, num_heads=num_heads)  # with skip connection
            self.scale = 0.5
        # ---------------------------------------------------
        self.dim = dim
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.window_size == 0:
            x, important_key = self.attn(x, return_aug=True)
            x = self.Space_Adapter(x, important_key)
        ...
        if self.window_size == 0:
            x = x + self.scale * self.MLP_Adapter(xn, important_key)
...
class ParaBlock(nn.Module):
    ...
    def __init__(
        ...
        if window_size == 0:
            ...
            #self.channel_Adapter = CrossChannelAttention(in_planes=dim)           
            self.scale = 0.5
        # ---------------------------------------------------
        self.dim = dim
    ...
    def forward(self, x: torch.Tensor, cnnx: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.window_size == 0:
            x = x + self.scale * self.MLP_Adapter(xn)
            #cnnx = self.channel_Adapter(x, cnnx)
...
class AugAttention(nn.Module):
    ...
    def forward(self, x, return_aug=False):
        ...
        x = self.proj(x)
        if return_aug:
            important_key = torch.sum(attn, dim=1).view(B, self.num_heads, H, W).permute(0, 2, 3, 1)  # (b*nHead 1 H*w) -> (b H,  nHead)
            return x, important_key
        else:
            return x
...
class CrossChannelAttention(nn.Module):
    ...
    def forward(self, x1, x2):
        # x1 = (b h w d), x2 = (b h w d)
        b, h, w, d = x1.shape
        x1 = x1.view(b, h*w, d).permute(0, 2, 1)
        avg_out = self.fc(self.avg_pool(x1).permute(0, 2, 1))
        max_out = self.fc(self.max_pool(x1).permute(0, 2, 1))
        out = avg_out + max_out # (b 1 d)
        out = self.sigmoid(out)
        x2 = x2.view(b, h*w, d) * out
        return x2.view(b, h, w, d)
